...
#   patch      build (or pull, or load) the new image, switch the deployment to it, recreate server + worker
...
# How many times to bring server and worker up before calling the server unhealthy.
PATCH_ATTEMPTS=${PATCH_ATTEMPTS:-3}
...
Options:
  ...
  --mode=MODE          where the patched image comes from: build, pull or load (default: build)
...
--mode=load compiles nothing here: it takes an image that was built on a bigger
machine and already restored by `docker load`, which is the only option on a host
too small for the front-end build (scripts/build-image.sh produces the archive).
Because the default tag names an image this host never built, --tag is required.
...
preflight() {
  ...
  if [ -z "$PATCH_TAG" ]; then
    # Any default would name an image this host never built, so the tag has to be
    # the one the transferred archive was loaded under.
    [ "$PATCH_MODE" != "load" ] ||
      die "--mode=load needs --tag: it must match the tag of the image already loaded here"
    local sha
...
# True only while the container is actually running. A crash loop leaves it in
# "restarting", which is the state worth giving up on.
container_is_running() { [ "$(container_state "$1")" = running ]; }

# Waits for the container's healthcheck to pass.
#
# An `unhealthy` reading is not on its own a reason to give up. The server
# healthcheck is `interval: 5s, retries: 20` with no start_period, so ~100s of
# booting - which is what a first start that is still applying database upgrades
# looks like - is enough to be marked unhealthy. Docker keeps running the check
# behind that, so the container recovers the moment it starts answering. Only a
# container that is not running at all is hopeless. HEALTH_TIMEOUT bounds the wait.
wait_for_health() {
    ...
    case "$status" in
      ...
      unhealthy)
        if ! container_is_running "$container"; then
          warn "the container is not running (state: $(container_state "$container"))"
          return 1
        fi
        ;;
    esac
...
# Brings server and worker up against whatever TAG now says, and waits for the
# server to answer. Returns non-zero only when the server is still unhealthy after
# PATCH_ATTEMPTS, so the caller decides whether that is fatal.
#
# The `up` itself is allowed to fail. The worker declares a dependency on the
# server's healthcheck, so compose reports "dependency failed to start: ... is
# unhealthy" the moment a first boot that is still running upgrades is marked
# unhealthy - while the healthcheck keeps re-running behind it. Treating that as
# fatal is what turned a slow boot into a failed patch.
up_and_wait() {
  local attempt=1 healthy=0

  while :; do
    compose up -d server worker ||
      warn "docker compose up reported a failure (attempt $attempt/$PATCH_ATTEMPTS)"

    SERVER_CID=$(compose ps -q server)
    WORKER_CID=$(compose ps -q worker)

    if [ -n "$SERVER_CID" ] && wait_for_health "$SERVER_CID"; then
      healthy=1
      break
    fi

    [ "$attempt" -lt "$PATCH_ATTEMPTS" ] || break

    # `up` does not restart a container that is merely unhealthy, so the retry has
    # to restart it. The next `up` then starts the worker it was blocking.
    warn "attempt $attempt/$PATCH_ATTEMPTS: the server is not healthy; restarting it"
    compose logs --tail=20 server 2>&1 || true
    compose restart server 2>&1 || true
    attempt=$((attempt + 1))
  done
  ...
  warn "the server was still not healthy after $PATCH_ATTEMPTS attempt(s); last 50 log lines:"
  compose logs --tail=50 server 2>&1 || true
  return 1
}
...
# The image was built and checked on another machine and restored by `docker load`,
# so nothing is compiled here: the tag only has to resolve to a local image of the
# architecture this host runs before compose is pointed at it.
use_loaded_image() {
  local image="$PATCH_IMAGE_REPO:$PATCH_TAG" image_arch

  docker image inspect "$image" >/dev/null 2>&1 ||
    die "no local image $image; transfer and 'docker load' it first (scripts/build-image.sh writes the archive)"

  # An image for the other architecture still pulls in and then crash-loops, which
  # is indistinguishable from the patch having broken the instance.
  image_arch=$(docker image inspect -f '{{.Architecture}}' "$image")
  case "$image_arch" in
    arm64 | aarch64) image_arch=linux/arm64 ;;
    amd64 | x86_64) image_arch=linux/amd64 ;;
  esac
  [ "$image_arch" = "$PLATFORM" ] ||
    die "$image is $image_arch but this host runs $PLATFORM"

  ok "using the loaded image $image ($(docker image inspect -f '{{.Id}}' "$image" | cut -c8-19))"
}
...
apply_patch() {
  ...
  elif [ "$PATCH_MODE" = "load" ]; then
    use_loaded_image
  else
    die "unsupported PATCH_MODE: $PATCH_MODE (expected build, pull or load)"
  fi
...
run_rollback() {
  ...
  if up_and_wait; then
    ...
    warn "the server is still not healthy at $previous_tag"
    return 1
  fi
